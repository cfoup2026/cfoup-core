...
export type SourceSystem = 'bank' | 'accounts_receivable' | 'accounts_payable' | 'erp' | 'accounting' | 'invoice' | 'card' | 'sales' | 'manual';
...
export type PaymentChannel = 'pix' | 'ted' | 'doc' | 'boleto' | 'check' | 'card' | 'cash' | 'transfer' | 'deposit' | 'wire' | 'unknown';
/**
 ...
 */
export type Bucket = 'receita' | 'deducoes' | 'custos_diretos' | 'folha' | 'despesas_operacionais' | 'caixa' | 'contas_receber' | 'contas_pagar' | 'despesas_financeiras' | 'retiradas_socios' | 'investimentos' | 'estoque';
 ...
 */
export type ClassificationStatus = 'classified' | 'translated' | 'needs_confirmation' | 'pending' | 'ignored';
/** Por que uma transação virou exceção e foi pra Pendências de Setup. */
export type ExceptionReason = 'generic_original_category' | 'unknown_counterparty' | 'bank_only_weak_description' | 'possible_transfer' | 'card_payment_without_detail' | 'possible_duplicate' | 'unmatched_bank_transaction' | 'large_other_category' | 'low_confidence' | 'accounting_generic_account' | 'receivables_advance' | 'loan_needs_breakdown' | 'refund_or_chargeback' | 'none';
/** Cardinalidade de um match de reconciliação banco × CR/CP. */
export type ReconciliationMatchType = 'one_to_one' | 'one_to_many' | 'many_to_one' | 'many_to_many' | 'partial';
/** Estado de um `ReconciliationMatch`. */
export type ReconciliationStatus = 'matched' | 'needs_confirmation' | 'unmatched' | 'ignored';
/** Como um match foi encontrado — provenance da reconciliação. */
export type ReconciliationMatchReason = 'same_amount_same_date' | 'same_amount_near_date' | 'counterparty_similarity' | 'batch_total_match' | 'document_number_match' | 'manual_owner_match';
/** Tipo de regra criada pela empresa para classificar transações futuras. */
export type RuleType = 'counterparty' | 'keyword' | 'original_account' | 'cost_center' | 'payment_channel' | 'amount_pattern' | 'accounting_account';
