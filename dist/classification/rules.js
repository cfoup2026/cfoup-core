...
function substringMatch(field, pattern) {
    if (field === undefined)
        return false;
...
const MATCHERS = {
    ...
    keyword: {
        matches: (r, t) => substringMatch(t.description, r.pattern) ||
            substringMatch(t.originalCategory, r.pattern) ||
            substringMatch(t.originalClassificationRaw, r.pattern),
    },
    original_account: {
        matches: (r, t) => substringMatch(t.originalAccountName, r.pattern) ||
    ...
    payment_channel: {
        matches: (r, t) => t.paymentChannel !== undefined &&
 ...
 */
export function applyClassificationRules(transaction, rules) {
    ...
    for (const rule of rules) {
        ...
        const result = {
            ...
            classificationMethod: rule.createdBy === 'owner' ? 'owner_confirmed' : ruleMethod(rule.ruleType),
 ...
 */
export function createRuleFromOwnerConfirmation(groupedException, selectedCategoryCode, options = {}) {
    const ruleType = inferRuleType(groupedException);
    const pattern = groupedException.groupLabel.trim();
    if (pattern === '')
        return null;
    if (getCategoryByCode(selectedCategoryCode) === undefined)
        return null;
