...
export function normalizeTransaction(transaction) {
    const norm = (s) => s === undefined ? null : normalizeText(s);
    return {
 ...
 */
export function detectGenericCategory(transaction) {
    const fields = [
...
export function detectTransfer(transaction) {
    const fields = [
        transaction.description,
        transaction.originalCategory,
        transaction.originalAccountName,
 ...
 */
export function detectCardPaymentWithoutDetail(transaction) {
 ...
 */
export function translateAccountingTransaction(transaction) {
    const candidates = [];
    ...
    const fallbackLabel = transaction.originalAccountName ??
        transaction.originalGroupName ??
...
function applyKeywordHeuristics(transaction) {
...
function buildHaystack(transaction) {
    const parts = [];
    const push = (s) => {
        if (s !== undefined)
 ...
 */
function classifyAR(transaction) {
    const haystack = buildHaystack(transaction);
    if (haystack.includes('adiantamento') ||
        haystack.includes('antecipacao cliente')) {
        return makeClassified(transaction, 'IN_CUSTOMER_ADVANCE', 'source_mapping', 0.88, 'receivables_advance');
    }
    // CR liquidado tem alta confiança; CR ainda em aberto, média.
    const score = transaction.paidDate !== undefined ? 0.92 : 0.7;
    return makeClassified(transaction, 'IN_CUSTOMER_RECEIPT', 'source_mapping', score);
}
function makeClassified(transaction, code, method, score, exceptionReason = 'none', extra = {}) {
    const cat = getCategoryByCode(code);
    const level = calculateConfidenceLevel(score);
    const status = level === 'high' ? 'classified' : 'needs_confirmation';
    const result = {
        sourceTransactionId: transaction.id,
        companyId: transaction.companyId,
    ...
    if (label !== undefined)
        result.ownerFriendlyLabel = label;
    if (extra.notes !== undefined)
        result.notes = extra.notes;
    return result;
}
function makePending(transaction, code, score, exceptionReason, method = 'fallback') {
    ...
    if (code !== null)
        result.standardCategoryCode = code;
 ...
 */
function resolveAccountCodeHint(code, hintMap) {
    if (hintMap.exact !== undefined) {
        const cat = hintMap.exact[code];
        if (cat !== undefined)
            return { category: cat, isExact: true };
    }
 ...
 */
function makeAccountCodeHintResult(transaction, code, isExact, heuristicAgrees, heuristicContradicts) {
    let score;
    let status;
    if (isExact) {
 ...
 */
export function classifyTransaction(transaction, options = {}) {
    /* 1. Regras da empresa */
    ...
    if (detectTransfer(transaction)) {
        const code = transaction.direction === 'inflow' ? 'IN_TRANSFER' : 'OUT_TRANSFER';
    ...
    if (options.reconciliationCategoryCode !== undefined) {
        return makeClassified(transaction, options.reconciliationCategoryCode, 'reconciliation_match', 0.92);
    }
    ...
    if (transaction.sourceSystem === 'sales') {
        if (transaction.direction === 'outflow') {
            ...
            return makeClassified(transaction, 'OUT_REFUND_CUSTOMER', 'source_mapping', 0.9);
        }
        // Venda regular (NF emitida): receita reconhecida no DRE accrual.
        // Score alto e determinístico — vendas sempre dispatcham aqui.
        return makeClassified(transaction, 'IN_INVOICED_REVENUE', 'source_mapping', 0.92);
    }
    /* 8. Account code hints — sinal externo opcional sobre originalAccountCode */
    if (options.accountCodeHints !== undefined &&
        transaction.originalAccountCode !== undefined) {
        const hint = resolveAccountCodeHint(transaction.originalAccountCode, options.accountCodeHints);
        if (hint !== null) {
            ...
            const heuristicAgrees = heuristic !== null && heuristic.code === hint.category;
            const heuristicContradicts = heuristic !== null && heuristic.code !== hint.category;
            return makeAccountCodeHintResult(transaction, hint.category, hint.isExact, heuristicAgrees, heuristicContradicts);
        }
    }
    /* 9. Heurísticas */
    const heuristic = applyKeywordHeuristics(transaction);
    if (heuristic !== null) {
        ...
        if (detectGenericCategory(transaction)) {
            return makePending(transaction, heuristic.code, Math.min(heuristic.score, 0.55), 'generic_original_category', heuristic.method);
        }
        return makeClassified(transaction, heuristic.code, heuristic.method, heuristic.score);
    }
    /* 8. Genérico sem heurística */
    if (detectGenericCategory(transaction)) {
        const fallback = transaction.direction === 'inflow' ? 'IN_OTHER' : 'OUT_OTHER';
        return makePending(transaction, fallback, 0.4, 'generic_original_category');
    }
    /* 9. Banco sem match */
    if (transaction.sourceSystem === 'bank') {
        const fallback = transaction.direction === 'inflow' ? 'IN_OTHER' : 'OUT_OTHER';
        return makePending(transaction, fallback, 0.35, 'unmatched_bank_transaction');
    }
    /* 10. Fallback final */
    const fallback = transaction.direction === 'inflow' ? 'IN_OTHER' : 'OUT_OTHER';
