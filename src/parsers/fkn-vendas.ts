 ...
 */
export function parseFKNVendas(rows: string[][]): ParseFKNVendasResult {
  const ok: Sale[] = [];
  const aggregates: SaleAggregate[] = [];
  const errors: ParseError[] = [];
  const warnings: ParseWarning[] = [];

  let headerSeen = false;
  let currentCustomer: { code: number; name: string } | null = null;
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (RULER_RE.test((row[0] ?? '').trim())) continue;
  ...
  if (!headerSeen) {
    errors.push({
      ...
      raw: '',
      reason:
        'cabeçalho FKN Vendas não encontrado (esperava DATA;NOTA;VENDEDOR;PRAZO;VALOR NOTA;VALOR CUSTO;%LUC)',
    });
  }
...
function isHeaderRow(row: readonly string[]): boolean {
  ...
  for (let i = 0; i < EXPECTED_HEADERS.length; i++) {
    if ((row[i] ?? '').trim() !== EXPECTED_HEADERS[i]) return false;
  }
  return true;
}
...
function parseSaleRow(
  cols: readonly string[],
  rawCols: readonly string[],
  currentCustomer: { code: number; name: string } | null,
  lineNumber: number,
  raw: string,
  errors: ParseError[],
  ...
  let movementType: SaleMovementType = 'sale';
  let movementTypeSource: SaleMovementTypeSource = 'explicit';
  if (signedAmount < 0) {
    movementType = 'return';
  ...
  const { marginPercent, marginPercentSource } = resolveMargin(
    csvMargin,
    amount,
    cost,
  );
...
function resolveMargin(
  ...
  amount: number,
  cost: number,
): { marginPercent: number | null; marginPercentSource: MarginPercentSource } {
  if (csvValue !== null) {
