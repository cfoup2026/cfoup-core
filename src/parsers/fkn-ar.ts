...
import type { PaymentStatus } from '../types/payable.js';
...
const REPORT_FOOTER_RE =
  /^(TOTAL\s+(GERAL|L[IÍ]QUIDO)|DESCONTADOS|CAUCIONADOS|OUTROS|A\s+VISTA[,\s]|Obs:?)/i;
 ...
 */
export function parseFKNAr(rows: string[][]): ParseFKNArResult {
  const ok: Receivable[] = [];
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (REPORT_FOOTER_RE.test(col1)) continue;
    if (LEGEND_LINE_RE.test(col0)) continue;
    ...
    const receivable = parseReceivableRow(
      trimmed,
      lineNumber,
      raw,
      errors,
  ...
  if (!headerSeen) {
    errors.push({
      ...
      raw: '',
      reason:
        'cabeçalho FKN AR não encontrado (esperava EMIS;COD.;CLIENTE;FIL;DUPLIC.;ID;VALOR;...)',
    });
  }
...
function computeStatus(amount: number, amountPaid: number): PaymentStatus {
  if (amountPaid <= STATUS_EPSILON) return 'open';
  if (amountPaid > amount + STATUS_EPSILON) return 'overpaid';
  if (Math.abs(amountPaid - amount) <= STATUS_EPSILON) return 'paid';
  return 'partial';
}
