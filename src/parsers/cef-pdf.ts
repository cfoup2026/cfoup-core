// TODO [debt]: Lógica de extração PDF está acoplada ao parser CEF.
// Quando entrar segundo banco (Bradesco/Itaú), refatorar em 2 camadas:
//   Camada 1 — pdf/extractor.ts (adapter genérico)
//   Camada 2 — parsers/{banco}-pdf.ts (regras específicas)
// Estimativa: 4-8h de refatoração.
import type {
  Transaction,
  TransactionDirection,
} from '../types/transaction.js';
import type { BalanceSnapshot } from '../types/balance.js';
import type {
  ParseError,
  ParseResult,
  ParseWarning,
...
const TX_RE =
  /^(\d{2}\/\d{2}\/\d{4})\s+(\d{6})\s+(.+?)\s+([\d.]+,\d{1,2})\s+([CD])$/;
const SALDO_INTERCALADO_RE = /^Saldo\s+([\d.]+,\d{1,2})\s+([CD])$/i;
const SALDO_ANTERIOR_RE =
  /^SALDO\s+ANTERIOR\s+([\d.]+,\d{1,2})(?:\s+([CD]))?$/i;
const HEADER_RE =
  /^(?:NR\.|DOC\.|DATA MOV\.|HIST.?RICO|VALOR|Extrato(?: por per.?odo)?|DATA MOV\..*HIST.?RICO.*VALOR)$/i;
const FOOTER_RE = /^\*\s*\d+/;
 ...
 */
export async function parseCEFPdf(
 ...
 */
export function parseCEFPdfFromLines(
  lines: ExtractedLine[],
  ...
  if (accountId === '') {
    warnings.push({
      ...
      message:
        'número da conta não localizado no PDF — Transaction.accountId fica vazio',
    });
  }

  type Pending = {
    kind: 'anterior' | 'opening';
    amount: number;
    lineIndex: number;
  };
  ...
  for (const line of lines) {
    ...
    if (/^DATA MOV\..*VALOR$/i.test(text)) continue;
    ...
    if (txMatch !== null) {
      ...
      const isSaldoDia =
        docNumber === SALDO_DOC_NUMBER && SALDO_DIA_HISTORY_RE.test(history);
      if (isSaldoDia) {
        balances.push({
          accountId,
          date,
  ...
  if (pending.length > 0) {
    warnings.push({
      ...
      message:
        'PDF sem transações: saldos pendentes (SALDO ANTERIOR/abertura) descartados',
    });
  }
...
function flushPending(
  pending: { kind: 'anterior' | 'opening'; amount: number; lineIndex: number }[],
  firstTxDate: Date,
  accountId: string,
  ...
  for (const p of pending) {
    const date =
      p.kind === 'anterior' ? addUTCDays(firstTxDate, -1) : firstTxDate;
    balances.push({
      accountId,
      date,
      amount: p.amount,
...
function parseDirection(debCred: string): TransactionDirection | null {
...
function findAccountId(lines: readonly ExtractedLine[]): string {
  for (const line of lines) {
