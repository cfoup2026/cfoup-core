// TODO [debt]: Lógica de extração PDF segue acoplada ao parser específico.
// Quando entrar 2º contador (não-Kinlex), promover pra estrutura genérica
// conforme TODO em src/parsers/cef-pdf.ts. Hoje DRE e Balanço já compartilham
// padrões (header Empresa/CNPJ/título, ruler de assinatura, parens negativos);
// a refatoração natural extrai um KinlexHeaderExtractor reutilizável.
import type {
  BalanceSheetEntry,
  BalanceSheetLineItem,
  BalanceSheetSubtotal,
  BalanceType,
} from '../types/balance-sheet.js';
import type {
  ParseError,
  ParseResult,
  ParseWarning,
...
const EMPTY_METADATA: KinlexBalancoMetadata = {
  companyName: '',
  cnpj: '',
  referenceDate: null,
  period: null,
 ...
 */
export async function parseKinlexBalanco(
  input: Uint8Array | ArrayBuffer | Buffer,
): Promise<ParseKinlexBalancoResult> {
  let lines: ExtractedLine[];
  try {
    lines = await extractTextLines(input);
  } catch (err) {
    const reason =
      err instanceof Error ? err.message : 'falha ao extrair texto do PDF';
    return {
      ok: [],
      balances: [],
...
export function parseKinlexBalancoFromLines(
  lines: ExtractedLine[],
  ...
  const bodyLines: { line: ExtractedLine; match: RegExpExecArray }[] = [];
        ...
        errors: [
          {
            ...
            raw: line.text,
            reason:
              'Balanço requer xStart pra detectar hierarquia. Atualize extractTextLines pra popular xStart antes de parsear Balanço.',
          },
        ],
        warnings,
        metadata,
  ...
  for (const { line, match } of bodyLines) {
    ...
    preEntries.push({
      ...
      label: labelRaw,
      amount,
      balanceType: dc as BalanceType,
      level,
      sectionPath,
    });
  }

  const ok: BalanceSheetEntry[] = [];
  for (let i = 0; i < preEntries.length; i++) {
    ...
    if (hasChild) {
      const sub: BalanceSheetSubtotal = {
        id,
        kind: 'subtotal',
        label: cur.label,
        level: cur.level,
...
function isHeaderLine(text: string): boolean {
  if (text.startsWith('Empresa:')) return true;
  if (text.startsWith('C.N.P.J.:')) return true;
  if (/^Período:/i.test(text)) return true;
  if (/^Balanço\s+encerrado\s+em:/i.test(text)) return true;
  if (TITLE_RE.test(text)) return true;
  if (COLUMN_HEADER_RE.test(text)) return true;
  if (text.startsWith('Folha:')) return true;
  if (text.startsWith('Emissão:')) return true;
  if (text.startsWith('Hora:')) return true;
  return false;
}

function extractMetadata(lines: readonly ExtractedLine[]): KinlexBalancoMetadata {
  let companyName = '';
  let cnpj = '';
  let title = '';
  ...
  for (const line of lines) {
    ...
    if (companyName === '') {
      ...
      if (m !== null) companyName = (m[1] ?? '').trim();
    }
    if (cnpj === '') {
      const m = CNPJ_RE.exec(text);
      if (m !== null) cnpj = (m[1] ?? '').trim();
    }
    if (period === null) {
      ...
      if (m !== null) {
        ...
        if (start !== null && end !== null) period = { start, end };
      }
    }
    if (referenceDate === null) {
      const m = REFERENCE_DATE_RE.exec(text);
      if (m !== null) referenceDate = parseDDMMYYYYtoUTC(m[1] ?? '');
    }
    if (title === '' && TITLE_RE.test(text)) title = text.trim();
  }
 ...
 */
function buildXToLevelMap(xs: readonly number[]): Map<number, number> {
  const unique = new Set<number>();
  for (const x of xs) unique.add(roundX(x));
