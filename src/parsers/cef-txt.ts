 ...
 */
export function parseCEFTxt(content: string): ParseResult<Transaction> {
  ...
  for (let idx = 0; idx < rawLines.length; idx++) {
    ...
    const [accountId, dateStr, docNumber, history, valueStr, debCred] =
    ...
    const isSaldoDia =
      docNumber === SALDO_DOC_NUMBER && SALDO_HISTORY_PATTERN.test(history);
    if (isSaldoDia) {
      balances.push({
        accountId,
        date,
...
function isHeaderRow(fields: readonly string[]): boolean {
  ...
  for (let i = 0; i < EXPECTED_HEADERS.length; i++) {
    if (fields[i] !== EXPECTED_HEADERS[i]) return false;
  }
  return true;
}

function parseAmount(valueStr: string): number | null {
