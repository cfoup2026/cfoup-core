...
// Quando entrar 2º banco (CEF refactor) e/ou 2º contador (não-Kinlex),
// promover pra estrutura genérica conforme o TODO em src/parsers/cef-pdf.ts.
import type {
  DREEntry,
  DRELineItem,
  DRESectionHeader,
...
const KNOWN_SUBTOTAL_LABELS: ReadonlySet<string> = new Set([
  'RECEITA LÍQUIDA',
  'LUCRO BRUTO',
...
const SKIP_HEADER_RE =
  /^(Folha:|Número\s+livro:|C\.N\.P\.J\.:|Empresa:)/i;

const EMPTY_METADATA: KinlexDREMetadata = {
  companyName: '',
  cnpj: '',
  referenceDate: null,
  title: '',
 ...
 */
export async function parseKinlexDRE(
  input: Uint8Array | ArrayBuffer | Buffer,
): Promise<ParseKinlexDREResult> {
 ...
 */
export function parseKinlexDREFromLines(
  lines: ExtractedLine[],
  ...
  for (const line of lines) {
    ...
    if (SKIP_HEADER_RE.test(text)) continue;
    ...
    if (v2 !== null && !twoColumnWarningEmitted) {
      warnings.push({
        line: lineNumber,
        message:
          'DRE Kinlex tem 2 colunas numéricas sem rótulo no cabeçalho — preservando como value1/value2; significado a ser determinado pelo consumidor',
      });
      twoColumnWarningEmitted = true;
    }

    const lineItem: DRELineItem = {
      id: `kinlex-dre:${lineNumber}`,
      kind: 'line_item',
      label: labelText,
      section: currentSection,
...
function extractMetadata(lines: readonly ExtractedLine[]): KinlexDREMetadata {
  let companyName = '';
  let cnpj = '';
  let title = '';
...
function extractValues(text: string): ExtractedValue[] {
 ...
 */
function canonicalizeLabel(s: string): string {
  return s
    .replace(/^\(-\)\s*/, '')
    .replace(/\s+/g, ' ')
    .trim()
