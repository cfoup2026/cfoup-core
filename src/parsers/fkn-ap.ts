import type { Payable, PaymentStatus } from '../types/payable.js';
 ...
 */
export function parseFKNAp(rows: string[][]): ParseFKNApResult {
  const ok: Payable[] = [];
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    if (LEGEND_LINE_RE.test(trimmed[0] ?? '')) continue;
